...
全站一套暗色主题，落地页、登录页、控制台和编辑器共用同一批 shadcn token，没有第二套调色板：

- **石墨灰机身**（`--graphite-950/900/850/800`）。UI 退到背景里，因为整个界面上最该亮的是幻灯片本身——
  它是白的，是用户真正做出来的东西。编辑器画布地板另有一档更深的 `--stage`，让幻灯片浮在光里；
  幻灯片边缘用 `ring-white/15` 而不是深色描边，否则在暗背景上等于没有边。
- **骨白文字**（`--bone`）而不是纯白，**电光青柠**（`--volt`）是唯一强调色，且刻意用得很省：
  只给主按钮、聚焦环和选中态。
- 圆角收到 `--radius: 0.25rem`，这是台仪器，不是卡片墙。

`:root` 就是暗色本体，没有 `.dark` 块；`<html>` 上挂 `dark` 只是为了让 shadcn 组件自带的
`dark:` 分支生效——那些分支本来就是为暗色表面写的。所以**改主题不需要动任何组件**，
编辑器的工具栏、面板、对话框全是跟着 token 走的。
...
`src/**/*.test.ts`，jsdom 环境，174 个用例。覆盖 HTML 消毒（含越权提升的回归用例）、几何与旋转、
颜色转换、富文本 run 拆分、deck 迁移、自由绘制路径、动画分步、图片烘焙的降级路径、
