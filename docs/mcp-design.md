...
- 一个"在编辑器中打开"按钮（只对能编辑的人显示——拿着分享链接的路人点进去只会撞到 404）
- **一个「跟随改动」开关，默认开，手动选页即关**（实现时补的）。没有它，一个人回到第 1 页细看时会被每 1.5 秒拽走一次——比让他自己点还糟。关掉后内容照常更新，只是视线不动；再打开会跳到最后一页，否则按钮看起来像没反应。

页面是 `ssr: false` 的，理由比编辑器多一条：幻灯片是用 `dangerouslySetInnerHTML` 从存储的 HTML 渲染的，而让这件事安全的重新消毒需要真实 DOM——服务端渲染这一页等于在任何东西擦洗它之前就把存储的标记吐出去。

**两条进门的路：会话 cookie，或者链接自带的签名**（实现时补的）。原本这里写的是"走 `currentUser()`，和 dashboard 同一套 cookie"，但那让 agent 交出去的链接在真实场景里是废的：对面的人常常没登录，甚至没有账号，一个先要求 Google 登录的预览链接不是链接，是绕路。

所以 MCP 工具返回的 `previewUrl` 带一个签名参数 `?k=`（[`web/src/lib/share-link.ts`](../web/src/lib/share-link.ts)）：

- **签的内容**：`v1.deckId.ownerId.到期日` 的 HMAC-SHA256，密钥就是 `AUTH_SECRET`——签会话 cookie 的那把。轮换密钥同时废掉 cookie 和链接，是一个动作而不是两个
- **只读，且只读一份**：deckId 在签名里，所以一份链接换不到第二份稿子；只有 `GET /api/decks/[id]` 和 `GET .../version` 认这个 key，PUT / PATCH / DELETE 和缩略图上传一概不认——一个会在聊天窗口里被转发的 URL 不该能改稿子
- **一周有效，按整天取整**：每个写工具都返回 `previewUrl`，写二十页应该是同一个链接返回二十次，而不是二十个链接。代价是实际寿命在 6～7 天之间
- **不能单条撤销**：背后没有行，这是它扛得住每 1.5 秒一次轮询的原因（验签不查库）。要提前掐断只能等它过期或轮换 `AUTH_SECRET`。所以到期时间是唯一免费的杠杆——一周，不是一年
- **owner id 写在链接里**：`lib/decks.ts` 的每次读都在同一条 SQL 里按 owner 过滤，这条路不能成为唯一"先取行、再判归属"的例外
- 链接失效时给的是"这个链接已失效"，不是跳登录页——把一个没有账号的人送去 Google，是让他修一件本来就不归他修的事
- 页面 `noindex`：自带访问权的链接迟早会被贴到公开的地方
...
每个写工具的返回值都带 `previewUrl` 和新的 `version`。Agent 把链接给人，人在浏览器里实时看着生成过程——不用登录，见上一节的签名链接。`deck_preview` 的工具描述里明说了这一点，好让模型在递出链接时替用户说清楚：谁拿到谁能看，一周内有效。这是"人和 agent 共享一个预览界面"在本期的真实形态——**共享的是同一个 URL 和同一份状态，不是同一双眼睛**。
...
| 5 ✅ | `/preview/[id]` + 自动跳页 | 人开着预览页，看着 agent 一页页写出来（已实测：写第 2、3 页时主视图自动跟过去，全程不刷新）|
